...
export function buildOverviewOpportunities({
  ...
  const averageRoas = rows.reduce((sum, row) => sum + row.roas, 0) / rows.length;

  const scaleCandidate = [...rows].sort((a, b) => b.roas - a.roas)[0];
  const scaleShare = totalSpend > 0 ? (scaleCandidate.spend / totalSpend) * 100 : 0;

  const wasteCandidate = [...rows].sort((a, b) => b.spend - a.spend)[0];

  const recentTrend = data.trends["7d"];
  const first = recentTrend[0];
  const last = recentTrend[recentTrend.length - 1];
  const ctrProxyDrop = first && last && first.revenue > 0 ? ((last.revenue - first.revenue) / first.revenue) * 100 : 0;
  ...
  return [
    {
      ...
      impact: scaleCandidate.roas >= averageRoas * 1.15 ? "High" : "Med",
      summary: [
      ...
      evidence: [
        {
          ...
          current: scaleCandidate.roas.toFixed(2),
        ...
        },
        {
          ...
          current: formatCurrency(scaleCandidate.revenue),
    ...
    },
    {
      ...
      impact: wasteCandidate.roas < averageRoas * 0.9 ? "High" : "Med",
      summary: [
      ...
      evidence: [
        {
          label: "Spend",
          current: formatCurrency(wasteCandidate.spend),
          benchmark: "Highest platform spend",
        },
        {
          label: "ROAS",
          current: wasteCandidate.roas.toFixed(2),
          benchmark: `Avg ${averageRoas.toFixed(2)}`,
        },
        {
          label: "CPA",
          current: formatCurrency(wasteCandidate.cpa),
