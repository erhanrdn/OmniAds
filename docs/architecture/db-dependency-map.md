...
| `platform_overview_daily_summary` | Cross-provider daily projection backing overview fallbacks. | `lib/overview-summary-store`, Google/Meta refresh helpers | `lib/overview-summary-store`, Google projection fallback | Unique `(business_id, provider, provider_account_id, date)`; business/provider/date index | Indirect GET read-only |
| `platform_overview_summary_ranges` | Projection manifest and validity watermark per range. | `lib/overview-summary-store` | Google projection fallback validity checks | Unique `(business_id, provider, provider_account_ids_hash, start_date, end_date)` | Indirect GET read-only |
| `provider_reporting_snapshots` | Generic provider/cache table for GA4, Shopify, GAQL, internal route caches. | `lib/reporting-cache`, `lib/google-ads-gaql`, background/report warmers | same modules and route-cache helpers | Unique `(business_id, provider, report_type, date_range_key)` | Yes, but GET reads are now read-only |
...
| `seo_results_cache` | Cached SEO findings/overview windows. | `lib/seo/results-cache`, explicit SEO generation flows | SEO overview/findings routes | Unique `(business_id, cache_type, start_date, end_date)` | Yes, but GET reads are now read-only |
| `shopify_serving_state` | Current Shopify read-trust decision and divergence state. | `lib/shopify/read-adapter`, webhook sync route, admin override route | `lib/shopify/read-adapter`, `lib/shopify/status`, overview route | PK `(business_id, provider_account_id, canary_key)` via table design; business/range indexes | Yes, but GET reads are now read-only |
...
| `shopify_reconciliation_runs` | Persisted reconciliation snapshots between live/warehouse/ledger. | `lib/shopify/read-adapter`, `lib/shopify/warehouse` | status/read-adapter/admin health | business/recorded index | Yes, but GET reads are now read-only |
