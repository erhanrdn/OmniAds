...
| `platform_overview_summary_ranges` | Projection manifest and validity watermark per range. | `lib/overview-summary-materializer` via warehouse invalidation and `lib/overview-summary-range-owner` / `npm run overview:summary:materialize` | Google projection fallback validity checks | Unique `(business_id, provider, provider_account_ids_hash, start_date, end_date)` | Indirect GET read-only |
| `provider_reporting_snapshots` | Generic provider/cache table for GA4, Shopify, GAQL, internal route caches. | `lib/reporting-cache-writer`, `lib/sync/ga4-sync`, `lib/sync/shopify-sync`, `npm run reporting:cache:warm`, `lib/meta/cleanup`, out-of-scope Google Ads reset lanes | `lib/reporting-cache`, `lib/route-report-cache`, `lib/overview-service`, `lib/shopify/overview`, analytics routes | Unique `(business_id, provider, report_type, date_range_key)` | Yes, but GET reads are now read-only and writes are owner-gated |
...
| `seo_results_cache` | Cached SEO findings/overview windows. | `lib/seo/results-cache-writer`, `lib/sync/search-console-sync` | `lib/seo/results-cache`, SEO overview/findings routes | Unique `(business_id, cache_type, start_date, end_date)` | Yes, but GET reads are now read-only and writes are owner-gated |
...
| `shopify_reconciliation_runs` | Persisted reconciliation snapshots between live/warehouse/ledger. | `lib/shopify/overview-materializer`, `lib/sync/shopify-sync` | status/read-adapter/admin health | business/recorded index | Yes, but GET reads are now read-only and writes are owner-gated |
