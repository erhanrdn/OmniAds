...
| `provider_reporting_snapshots` | Generic provider/cache table for GA4, Shopify, GAQL, internal route caches. | `lib/reporting-cache-writer`, `lib/sync/ga4-sync`, `npm run reporting:cache:warm`, `lib/meta/cleanup`, out-of-scope Google Ads reset lanes | `lib/reporting-cache`, `lib/route-report-cache`, `lib/overview-service`, `lib/shopify/overview`, analytics routes | Unique `(business_id, provider, report_type, date_range_key)` | Yes, but GET reads are now read-only and writes are owner-gated |
