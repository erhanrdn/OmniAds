...
| `platform_overview_daily_summary` | Cross-provider daily projection backing overview fallbacks. | `lib/overview-summary-store`, Google/Meta refresh helpers | `lib/overview-summary-store`, Google projection fallback | Unique `(business_id, provider, provider_account_id, date)`; business/provider/date index | Indirect GET and write-on-read |
| `platform_overview_summary_ranges` | Projection manifest and validity watermark per range. | `lib/overview-summary-store` | Google projection fallback validity checks | Unique `(business_id, provider, provider_account_ids_hash, start_date, end_date)` | Indirect GET and write-on-read |
| `provider_reporting_snapshots` | Generic provider/cache table for GA4, Shopify, GAQL, internal route caches. | `lib/reporting-cache`, `lib/google-ads-gaql`, `lib/overview-service`, `lib/shopify/overview` | same modules and route-cache helpers | Unique `(business_id, provider, report_type, date_range_key)` | Yes and write-on-read |
...
| `seo_results_cache` | Cached SEO findings/overview windows. | `lib/seo/results-cache`, SEO routes | SEO overview/findings routes | Unique `(business_id, cache_type, start_date, end_date)` | Yes |
| `shopify_serving_state` | Current Shopify read-trust decision and divergence state. | `lib/shopify/read-adapter`, webhook sync route, admin override route | `lib/shopify/read-adapter`, `lib/shopify/status`, overview route | PK `(business_id, provider_account_id, canary_key)` via table design; business/range indexes | Yes and write-on-read |
...
| `shopify_reconciliation_runs` | Persisted reconciliation snapshots between live/warehouse/ledger. | `lib/shopify/read-adapter`, `lib/shopify/warehouse` | status/read-adapter/admin health | business/recorded index | Yes and write-on-read |
