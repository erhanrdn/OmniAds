...
- `deterministic_decision_engine`
  - Deterministic recommendations/decision-engine surface. Does not block initial page completeness.
...
| Campaign list | recommendation badge (`act` / `test` / `watch`) | Yes | page-scoped | Optional | No | `/api/meta/recommendations` | Same | `/api/meta/recommendations` | `buildMetaRecommendations`; badge selection in page | deterministic recommendation response | Yes | Badge hidden when there is no recommendation for that campaign | No | Deterministic decision-engine surface |
...
| Campaign detail | selected-campaign recommendation card | Yes | page-scoped | Optional | No | `/api/meta/recommendations` | Same | `/api/meta/recommendations` | `buildMetaRecommendations`; selection in `MetaCampaignDetail` | deterministic recommendation model | Yes | Card hidden when no recommendation exists for the selected campaign | No | Deterministic decision-engine surface |
...
| Decision OS overview | winner scale candidate count / headline | Yes | page-scoped | Optional | No | `/api/meta/decision-os` | Same | `/api/meta/decision-os` | `buildMetaDecisionOs`; `MetaDecisionOsOverview` | deterministic Meta Strategy Engine V2 summary | No | Falls back to route-provided summary or derived count | No | Additive V2-04 summary surface |
| Winner scale candidates board | candidate rows (campaign, ad set, move band, policy chips, confidence) | Yes | page-scoped | Optional | No | `/api/meta/decision-os` | Same | `/api/meta/decision-os` | `buildMetaDecisionOs`; `MetaDecisionOsOverview` | deterministic winner scale candidate list | Yes | Empty copy shown when no clean winner is ready | No | Read-only surface only; no queue mutation is introduced here |
| Campaign decision panel | strategy / objective / bid / primary-driver chips | Yes | page-scoped | Optional | No | `/api/meta/decision-os` | Same | `/api/meta/decision-os` | `buildMetaDecisionOs`; `MetaCampaignDecisionPanel` | deterministic campaign `policy` metadata | No | Chips render only when Decision OS is present | No | Additive V2-04 policy metadata |
