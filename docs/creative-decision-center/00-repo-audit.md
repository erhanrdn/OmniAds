...
|---|---|---|---|---|---|---|---|
| `lib/creative-decision-os.ts` | `CreativeDecisionAction` lines 66, `CreativeDecisionLifecycleState` lines 74-82, `CreativeDecisionPrimaryAction` lines 83-89, `CreativeDecisionOsCreative` lines 424-469, `CreativeDecisionOsV1Response` line 615, `buildCreativeDecisionOs` line 2961 | V1 snapshot, lifecycle, families, supply plan, operator policy attachment | Production-facing through snapshot route and Creative page | `scale_hard`, `scale_ready`, `promote_to_scaling`, `refresh_replace`, `block_deploy`, supply plan kinds | Many UI/API/services; see import graph in `01-vocabulary-mapping.md` | Family grouping, historical windows, benchmark reliability, fatigue, protected winners, supply plan, trust/provenance | High |
| `lib/creative-decision-os-v2.ts` | `CREATIVE_DECISION_OS_V2_PRIMARY_DECISIONS` lines 4-11, `CreativeDecisionOsV2Input` lines 30-53, `CreativeDecisionOsV2Output` lines 55-69, `resolveCreativeDecisionOsV2` line 209 | V2 primary decision, safety blockers, no-write queue/apply false | Preview/test/safety, not production default | `Scale`, `Cut`, `Refresh`, `Protect`, `Test More`, `Diagnose`; `creative`, `campaign-context`, `data-quality`, `insufficient-signal` | V2 preview/evaluation/scripts/tests | Safety gate: Scale review-only, Diagnose diagnose-only, no queue/apply writes | Medium |
...
| `lib/creative-operator-surface.ts` | quick filters lines 20-27, UI primary lines 31-37, subtone lines 39-44, reason tags lines 46-61, `resolveCreativeOperatorDecision` line 655, `buildCreativeOperatorItem` line 1160 | User-facing labels, buckets, quick filters, operator item | Production-facing UI vocabulary | `scale`, `test_more`, `protect`, `refresh`, `cut`, `diagnose`, `review_only`, `queue_ready` | Creative page, drawer, support, top section, detail | UI bucket logic, reason-label mapping, preview truth summary | High |
