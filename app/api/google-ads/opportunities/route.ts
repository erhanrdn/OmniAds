...
import { getGoogleAdsOpportunitiesReport } from "@/lib/google-ads/reporting";
...
export async function GET(request: NextRequest) {
  ...
  const cached = await getCachedRouteReport<Record<string, unknown>>({
    businessId,
    provider: "google_ads",
    reportType: "google_ads_opportunities",
    searchParams: request.nextUrl.searchParams,
  });
  if (cached) return NextResponse.json(cached);

  const report = await getGoogleAdsOpportunitiesReport({
    businessId,
    accountId,
    dateRange,
    customStart,
