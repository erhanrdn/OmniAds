...
function sanitizeErrorMessage(input: string): string {
  ...
  if (normalized.includes("unsupported_parameter")) {
    return "AI service rejected one request parameter for this model/account.";
  }
  ...
  if (normalized.includes("model") && normalized.includes("not")) {
    return "Configured AI model is not available for this API key/account.";
  }
  if (normalized.includes("unauthorized") || normalized.includes("authentication")) {
    return "AI service authentication failed for the configured API key.";
  }
  if (normalized.includes("timeout") || normalized.includes("timed out")) {
    return "AI service timed out while generating commentary.";
  }
...
export async function POST(request: NextRequest) {
  ...
  const messages = [
    ...
    { role: "user" as const, content: buildUserPrompt(report, currency) },
  ];
  ...
  try {
    ...
    return NextResponse.json({
      ...
      source: "fallback",
      commentary: fallback,
      warning: `${sanitizeErrorMessage(raw)} (${detail})`,
    });
  }
}
