...
async function main() {
  ...
  const { buildMetaPublishVerificationReport } =
  ...
  if (!businessId || businessId === "--help" || !providerAccountId || !day) {
    console.log(
      "usage: node --import tsx scripts/meta-verify-publish.ts <businessId> <providerAccountId> <day>",
    );
  ...
  console.log(JSON.stringify(buildMetaPublishVerificationReport(verification), null, 2));
}
